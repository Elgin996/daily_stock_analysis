...
| `REALTIME_SOURCE_PRIORITY` | Real-time quote source priority (comma-separated), e.g., `tencent,pytdx,akshare_sina,efinance,akshare_em`; add `tickflow` explicitly to use TickFlow realtime quotes | See .env.example | Optional |
| `A_SHARE_DAILY_SOURCE_PRIORITY` | Capability-specific A-share daily route; defaults to `tushare,tencent,pytdx,tickflow,baostock,akshare,efinance,yfinance`; unconfigured token-backed sources are skipped. | See .env.example | Optional |
| `EM_MIN_INTERVAL` / `EM_JITTER_MIN` / `EM_JITTER_MAX` | Shared throttle interval and jitter for project-owned EastMoney HTTP calls; local processes coordinate through SQLite. | `1.0` / `0.1` / `0.5` | Optional |
| `EM_RATE_LIMIT_STATE_PATH` | EastMoney throttle state file; point it at a shared volume for multi-container deployments. | System temp directory | Optional |
