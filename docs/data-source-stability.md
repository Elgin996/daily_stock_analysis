...
| 场景 | 已接入源 | 默认使用方式 | 失败处理 |
| --- | --- | --- | --- |
| A 股个股日线 / 技术面 | Tencent、Pytdx、Baostock、AkShare、Efinance、Tushare、TickFlow、YFinance | `A_SHARE_DAILY_SOURCE_PRIORITY` 按能力控制顺序（未配置时按全局 priority 动态排序）；已配置的 Tushare 仍可优先 | 单源失败后尝试下一个源；连续失败会短期熔断该源 |
...
flowchart TD
    ...
    C -->|命中且新鲜| COK[复用缓存]
    C -->|缺失或过期| DM{市场}
    DM -->|A 股个股/未登记标的| CN[Tushare if configured -> Tencent -> Pytdx -> TickFlow -> Baostock -> AkShare -> Efinance -> YFinance]
